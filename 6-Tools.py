...
def sales_after_discount(orig_price: float, discount_percentage: float ) -> float:
    ...
    Args:
        discount_percentage(float) : Percentage discount on the item e.g. A value of 20% discount on 100 should return 80
...
# TODO - Buggy - sometimes the model uses the tool even if the prompt is "Hi there!"
...
response = llm_with_tools.invoke("What would the final price for a member, if the original item  \
                           is priced at 200 and there is a 15 percent discount?")
...
print(sales_after_discount.invoke(args))